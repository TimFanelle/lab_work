...
STORE_PATH = '/media/djtface/Chonky/Programming/VS/Otterly/ValeroLabTesting/learningPPOandG2P'
    ...
    for _ in range(BATCH_SIZE):
        ...
        if done:
            ...
            if total_loss is not None:
                print(f"Episode: {episode}, latest episode reward: {episode_reward_sum}, "f"total loss: {np.mean(total_loss)}, critic loss: {np.mean(c_loss)}, "f"actor loss: {np.mean(act_loss)}, entropy loss {np.mean(ent_loss)}")
