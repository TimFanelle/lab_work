...
const unsigned int PACKET_SIZE = muscles + encodes;
...
int main()
{
    ...
    void *data_socket = zmq_socket(context, ZMQ_REP);
    const int rb = zmq_bind(data_socket, "tcp://*:4268");
    assert(rb == 0);
    
    int q = 1;
    while(True){
        /* Receiving client data */
        zmq_recv(data_socket, recvBuffer, 1024, 0);
        ...
        switch(option){
            ...
            case 3:
                ...
                zmq_send(data_socket, "Finished", 9, 0);
        ...
        strcpy(buffer, obs);
        len = strlen(obs);
        zmq_send(data_socket, buffer, len, 0);
