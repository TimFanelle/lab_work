...
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, Flatten, advanced_activations
...
def get_advantages(values, masks, rewards):
    ...
    for i in reversed(range(len(rewards))):
        ...
        gae = delta + gamma - lmda * masks[i] * gae
...
def get_model_actor_image(input_dims, output_dims):
    ...
    model.compile(optimizer=Adam(learning_rate=1e-4), loss=[ppo_loss(oldpolicy_probs=oldpolicy_probs,
...
while not target_reached and iters < max_iters:
    ...
    actor_loss = model_actor.fit([states, actions_probs, advantages, np.reshape(rewards, newshape=(-1, 1, 1)),
                                  values[:-1]], [(np.reshape(actions_onehot, newshape=(-1, n_actions)))],
                                 verbose=True, shuffle=True, epochs=8, callbacks=[tensor_board])
